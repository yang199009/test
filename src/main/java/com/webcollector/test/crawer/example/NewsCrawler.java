...
import com.webcollector.test.crawer.dao.JDBCHelper;
...
public class NewsCrawler extends BreadthCrawler {
     ...
     */
    public NewsCrawler(String crawlPath, boolean autoParse,JdbcTemplate template) {
    ...
    public void visit(Page page, CrawlDatums next) {
        ...
        if (page.matchUrl("http://blog.csdn.net/.*/article/details/.*")) {
            ...
            ArrayList<String> imgList = page.attrs("div[id=blog_userface] img", "src");
            Links links =  page.getLinks("div[id=blog_userface] a");
            
            if (jdbcTemplate != null) {
            	int updates=jdbcTemplate.update("insert into test"
            	        +" (url,title,author,imgs,links) values(?,?,?,?,?)",
            	        url,title, author, imgList.toString(),links.toString());
        	    if(updates==1){
        	        System.out.println("数据插入插入成功");
        	    }
			}
    ...
    public static void main(String[] args) throws Exception {
    	JdbcTemplate template = JDBCHelper.createJDBCTemplate("test", "jdbc:mysql://localhost/test?useUnicode=true&characterEncoding=utf8",
         		"root", "root", 5, 30);
        NewsCrawler crawler = new NewsCrawler("crawl", true,template);
        /*线程数*/
